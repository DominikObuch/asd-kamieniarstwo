    ...
    overwriteCurrent() {
        ...
        for (this.currentShown = this.lastShown; this.currentShown < this.containersEl.length + this.lastShown; this.currentShown++) { //this one for current article 
            if (this.data[this.currentShown] !== undefined) { //  image changing
                this.articleEl[3][i].src = "https://media.giphy.com/media/3oEjI6SIIHBdRxXI40/200w_d.gif";
                function loadImg(numberOfImg){
                    var downloadingImage = new Image();
                    downloadingImage.onload = function () {
                            pages.articleEl[3][numberOfImg].src = `${pages.imageSrc}${pages.lastShown+numberOfImg+1}.jpg` ;
                        }
                    downloadingImage.src = `${pages.imageSrc}${pages.lastShown+numberOfImg+1}.jpg`;
                }
                loadImg(i)
